...
pub async fn marketing_updates(name:&str,email:&str,topic:&str,body:&str,link:&str) -> Result<(),Box<dyn std::error::Error>>{

                fn build_marketing_html(name: &str, topic: &str, body: &str, link: &str) -> String {
                                format!(r#"
        <!DOCTYPE html>
<html>
<head>
...
<body style="font-family:Arial,sans-serif;background:#f9f9f9;">
  <table align="center" width="100%" style="max-width:600px;background:#fff;border-radius:10px;padding:20px;">
    <tr>
      <td align="center">
        <h2 style="color:#1b73e8;">{}!</h2>
        <p style="color:#555;">Hi {}, There is an exciting updaate for you!</p>

        {}
        <br/>
        <br/>
        <a href="{}" style="background:#1b73e8;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;">Join</a>
        ...
        "#, name, topic, body, link)
                };

        let html = build_marketing_html(topic, name, body, link);
